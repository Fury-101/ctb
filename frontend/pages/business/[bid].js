...
export default ({ business, bid }) => {
    ...
    return <>
        ...
        <Head>
            ...
            <title>Busy Bees - Business</title>
        </Head>
        <div
            className="-z-10 bg-cover absolute w-screen h-24"
            style={{
                backgroundImage:
                    `url(${process.env.strapiURL}${business?.data.attributes.banner?.data.attributes.url ?? ""})`
            }}
        ></div>
        ...
        <div className="m-auto left-0 right-0 text-center absolute">
            ...
            <p className="w-2/3 mx-auto text-lg font-light text-left block text-gray-100">
                {business.data.attributes?.description ?? ""}
            </p>
            <div className="flex flex-row mt-6 justify-around">
                {business.data.attributes.images?.data.map(element=>
                    <img
            ...
            <div className="text-center">
                <a href = {`/user/${business?.data.attributes?.user.data.id ?? ""}`} className="px-2 py-2 text-xl my-6 mx-2 text-ctbgray font-semibold rounded-lg bg-ctbaliengreen inline-block ">
                    About Me
                </a>
                <a href = {`/donate/${bid ?? ""}`} className="px-2 py-2 text-xl my-6 mx-2 text-ctbgray font-semibold rounded-lg bg-ctbaliengreen inline-block">
                    Donate
                </a>
            </div>
        </div>
